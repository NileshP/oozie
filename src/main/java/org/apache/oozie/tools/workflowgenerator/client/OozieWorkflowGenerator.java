...
public class OozieWorkflowGenerator implements EntryPoint {
    ...
    Logger logger = Logger.getLogger("GWTLOGGER");
    ...
    public Button createXMLButton() {
        ...
        btn.addClickHandler(new ClickHandler() {
            ...
            String xml = ((OozieDiagramController) controller).generateXml();
                
            
            final OrderConfirmation confirmation = new OrderConfirmation();
            confirmation.message=xml;

            InfoService.Util.getService().getInfo(confirmation,new MethodCallback<OrderConfirmation>() {

            	@Override
					public void onSuccess(Method method, OrderConfirmation response) {
					//	GWT.log("SUCCESS");
					//	System.out.print("Success");
					logger.log(Level.SEVERE, confirmation.message);
					}
					...
					logger.log(Level.SEVERE, "this message should get logged ERROR");
					}
				});
            }
        });
